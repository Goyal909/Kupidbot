...
async def balance(interaction: discord.Interaction, member: discord.Member = None):
    ...
    data["users"][uid] = bal
    save_data(data)
    await interaction.response.send_message(f"💳 {member.mention}'s Portfolio: **{bal} $KUPID**", ephemeral=True)
...
async def daily(interaction: discord.Interaction):
    ...
    save_data(data)
    await interaction.response.send_message(f"🎁 **Daily Stimulus:** +500 $KUPID added to your wallet!", ephemeral=True)
...
async def pay(interaction: discord.Interaction, member: discord.Member, amount: int):
    ...
    save_data(data)
    await interaction.response.send_message(f"💸 {interaction.user.mention} paid {member.mention} **{amount} $KUPID**.", ephemeral=True)
...
async def leaderboard(interaction: discord.Interaction):
    ...
    if not users:
        return await interaction.response.send_message("📭 No users found yet.", ephemeral=True)
...
async def create_market(interaction: discord.Interaction, question: str, opt_a: str, opt_b: str):
    ...
    data["active_markets"][m_id]["message_id"] = original.id
    save_data(data)
...
async def bet(interaction: discord.Interaction, market_id: str, amount: int, choice: str):
    ...
    if not market or market["status"] != "OPEN":
        return await interaction.response.send_message("❌ Market unavailable.", ephemeral=True)
    if choice not in market["options"]:
        valid = " or ".join(f"`{k}`" for k in market["options"].keys())
        return await interaction.response.send_message(f"❌ Invalid choice. Pick {valid}.", ephemeral=True)
    ...
    if amount > u_bal or amount <= 0:
        return await interaction.response.send_message("❌ Insufficient $KUPID.", ephemeral=True)
...
async def check_bet(interaction: discord.Interaction, market_id: str):
    ...
    if not market:
        return await interaction.response.send_message("❌ Market not found or already resolved.", ephemeral=True)
...
async def edit_bet(interaction: discord.Interaction, market_id: str, new_choice: str, new_amount: int):
    ...
    existing = next((b for b in market["bets"] if b["uid"] == uid), None)
    if not existing:
        return await interaction.response.send_message("❌ You don't have a bet on this market. Use `/bet` to place one.", ephemeral=True)
    ...
    if new_amount > u_bal:
        return await interaction.response.send_message(f"❌ Insufficient $KUPID. You have {u_bal} $KUPID available (including your refund).", ephemeral=True)
...
async def remove_bet(interaction: discord.Interaction, market_id: str):
    ...
    existing = next((b for b in market["bets"] if b["uid"] == uid), None)
    if not existing:
        return await interaction.response.send_message("❌ You don't have a bet on this market.", ephemeral=True)
...
async def resolve(interaction: discord.Interaction, market_id: str, winner: str):
    ...
    for w in winners:
        try:
            print(f"[DM] Attempting to DM winner {w['uid']} ({w['user_name']})")
            ...
            print(f"[DM] Successfully DMed winner {w['uid']}")
    ...
    for l in losers:
        try:
            print(f"[DM] Attempting to DM loser {l['uid']} ({l['user_name']})")
            ...
            print(f"[DM] Successfully DMed loser {l['uid']}")
        except Exception as e:
...
async def configure_bet_log(interaction: discord.Interaction, channel: discord.TextChannel):
    ...
    save_data(data)
    await interaction.response.send_message(f"✅ Bet activity log channel set to {channel.mention}.", ephemeral=True)
...
async def configure_role(interaction: discord.Interaction, cmd: str, role: discord.Role):
    ...
    save_data(data)
    await interaction.response.send_message(f"🔐 `{cmd}` now requires @{role.name}.", ephemeral=True)
...
async def export_txt(interaction: discord.Interaction):
    ...
    with io.BytesIO(report.encode('utf-8')) as f:
        await interaction.response.send_message("📄 **Text Export:** Here is your readable ledger.", file=discord.File(f, "kupid_ledger.txt"), ephemeral=True)
